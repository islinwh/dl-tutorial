...
class Bottleneck(nn.Module):
    def __init__(self, in_channels, mid_channels, out_channels, stride):
        ...
        self.conv2 = nn.Conv2d(mid_channels, mid_channels, kernel_size=3, padding=1, stride=stride)
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks_arr, num_classes):
        ...
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, padding=1) # not 7
        self.bn1 = nn.BatchNorm2d(64)
        self.max_pool = nn.MaxPool2d(3, stride=2)
        self.conv2 = self._make_layer(block, num_blocks_arr[0], 64, 64, 256, downsample=False)
        self.conv3 = self._make_layer(block, num_blocks_arr[1], 256, 128, 512, downsample=True)
        self.conv4 = self._make_layer(block, num_blocks_arr[2], 512, 256, 1024, downsample=True)
        self.conv5 = self._make_layer(block, num_blocks_arr[3], 1024, 512, 2048, downsample=True)
        self.avg_pool = nn.AvgPool2d(4)
        self.fc = nn.Linear(2048, num_classes)
        
    def _make_layer(self, block, num_blocks, in_channels, mid_channels, out_channels, downsample):
        layers = []
        if downsample == False:
            layers.append(block(in_channels, mid_channels, out_channels, 1))
            for i in range(num_blocks-1):
                layers.append(block(out_channels, mid_channels, out_channels, 1))
        else:
            layers.append(block(in_channels, mid_channels, out_channels, 2))
            for i in range(num_blocks-1):
                layers.append(block(out_channels, mid_channels, out_channels, 1))
